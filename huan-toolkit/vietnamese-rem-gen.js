...
for (i=0;i<t.length;i=i+2) { conv[t[i]] = t[i+1];}
	
var keys = [];
for (var key in conv) {
	if (conv.hasOwnProperty(key)) {
	keys.push(key);
	}
}
keys.sort ();
...
var ext = "";
var cnt = 0;
for (var i=parseInt("1ea0", 16); i<parseInt("1f00", 16); i++) {
	
	if (conv.hasOwnProperty(String.fromCharCode(i))) {
		ext += "/* " + conv[String.fromCharCode(i)] + " */ 0x" + pad(conv[String.fromCharCode(i)].charCodeAt(0).toString(16),4,"").toLowerCase() + ", "
	} else {
		ext += "0x" + pad(i.toString(16),4,"").toLowerCase() + ", "
	}
